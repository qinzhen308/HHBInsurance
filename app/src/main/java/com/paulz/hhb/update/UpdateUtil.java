...
public class UpdateUtil extends com.core.framework.update.UpdateUtil{
    ...
    private static final String TUAN800_NAME = "hkb.apk";
    private static final String ZHE800_NAME = "hkb.apk";
    ...
    public static void checkBackgroundDate(final Activity context, final boolean fromSettings, final boolean needUpdate) {
        ...
        pb.append("version", HApplication.getInstance().getVersionCode());
...
//                        }
    ...
    public static void checkZheUpdate(final String updateStr, final boolean fromSettings) throws JSONException {
        ...
        JSONObject root = new JSONObject(updateStr);
        JSONObject versions = root.optJSONObject("versions");
        JSONObject mJSONObject = versions.optJSONObject("and");
        ZheUpdateEntity mZheUpdateEntity = new ZheUpdateEntity();
        mZheUpdateEntity.appName = AppConfig.CLIENT_TAG;
        mZheUpdateEntity.downloadUrl = versions.optString("url");
//        mZheUpdateEntity.remoteVersionCode = Integer.valueOf(mJSONObject.optString("version_sort"));
        mZheUpdateEntity.remoteVersionCode = mJSONObject.optInt("value");
//        mZheUpdateEntity.remoteVersionCode = 1000;//test
//        mZheUpdateEntity.remoteMinVersionCode = Integer.valueOf(mJSONObject.optString("mix_version"));
        ...
        mZheUpdateEntity.description = mJSONObject.optString("body");
        mZheUpdateEntity.title = mJSONObject.optString("title");
        mZheUpdateEntity.mustUpdate = mJSONObject.optInt("type")==1;
//        mZheUpdateEntity.mustUpdate = true;//test
    ...
    private static void showUpdateDialog(final UpdateBuilder tuanUpdateBuilder, boolean isNew, final boolean fromSettings) {
        ...
        if (!AppUtil.isEmpty(partnerList)) {
            ...
            dialog.setTitleText(partnerList.get(0).partner.title);
                ...
                public void onPositiveClick() {
                    ...
                    for (UpdateBuilder builder : partnerList) {
                        if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.N){
                            ...
                            String url=builder.partner.downloadUrl;
//                            intent.addCategory(Intent.CATEGORY_BROWSABLE);
                            if(!url.startsWith("http")){
                                url="http://"+url;
                            }
                            LogUtil.d("----download-----downloadUrl="+builder.partner.downloadUrl);
                            Uri uri = Uri.parse(url);
                            intent.setData(uri);
                            mContext.startActivity(Intent.createChooser(intent,"选择工具下载apk"));
                        }else {
