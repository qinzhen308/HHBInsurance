...
public class CirclePageIndicator extends View implements PageIndicator {
    ...
    public CirclePageIndicator(Context context, AttributeSet attrs, int defStyle) {
        ...
        mPaintPageFill.setStrokeWidth(1);//画圆环的画笔
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int longPaddingRight = count*15;
        if (mRight) {
            //todo  1、具体尺寸待修改   2、牵一发而动全身，其他的Indicator该如何改
            longOffset += ((longSize - longPaddingBefore-(longPaddingAfter+longPaddingRight))) - ((count * threeRadius) / 2.0f);
        }else {
        ...
        float pageFillRadius = mRadius+1;
...
//        }
        ...
        for (int iLoop = 0; iLoop < count; iLoop++) {
            ...
            if (mPaintPageFill.getAlpha() > 0 && isNeedCircle) {
//                canvas.drawRect(dX, dY, dX + mRadius, dY + mRadius, mPaintPageFill);
                canvas.drawCircle(dX, dY, pageFillRadius, mPaintPageFill);
            }

            // Only paint stroke if a stroke width was non-zero
            if (pageFillRadius != mRadius && isNeedCircle) {
//                canvas.drawRect(dX, dY, dX + mRadius, dY + mRadius, mPaintStroke);
...
//        canvas.drawRect(dX, dY, dX + mRadius, dY + mRadius, mPaintFill);
