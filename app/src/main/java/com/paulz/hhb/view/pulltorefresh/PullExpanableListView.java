package com.paulz.hhb.view.pulltorefresh;
...
/**
 * 下拉刷新上拉加载的ExpandableListView
 * 
 * @author jjj
 * 
 * @time 2016-1-7
 */
public class PullExpanableListView extends
	...
	public PullExpanableListView(Context context, int mode) {
		super(context, mode);
		mContext = context;
		this.setDisableScrollingWhileRefreshing(false);
	}

	public PullExpanableListView(Context context, AttributeSet attrs) {
		super(context, attrs);
		mContext = context;
		this.setDisableScrollingWhileRefreshing(false);
	}
	...
	protected ExpandableListView createRefreshableView(Context context,
		...
		String releaseLabel = context
				.getString(R.string.pull_to_refresh_release);
		String upLabel = context.getString(R.string.up_to_refresh);

		boolean isShowHeader;
		TypedArray typedArray = context.obtainStyledAttributes(attrs,
				R.styleable.pullshowheader);
		isShowHeader = typedArray.getBoolean(
				R.styleable.pullshowheader_showheader, true);

		LogUtil.d("<----------PullSwipeListView isShowHeader----------->"
				+ isShowHeader);

		if (mode == MODE_PULL_DOWN_TO_REFRESH || mode == MODE_BOTH) {
			FrameLayout frame = new FrameLayout(context);
			mHeaderLoadingView = new PullLoadingLayout(context,
					MODE_PULL_DOWN_TO_REFRESH, releaseLabel, pullLabel,
					refreshingLabel, isShowHeader);
		...
		if (mode == MODE_PULL_UP_TO_REFRESH || mode == MODE_BOTH) {
			...
			mFooterLoadingView = new PullLoadingLayout(context,
					MODE_PULL_UP_TO_REFRESH, releaseLabel, upLabel,
					refreshingLabel, isShowHeader);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		// setMeasuredDimension(CouponApplication.WIDTH,
		// CouponApplication.HEIGHT);
	...
	public void setRefreshingInternal(boolean doScroll) {
		...
		// listViewLoadingLayout.setPullRefreshTime(
		// mContext.getString(R.string.pull_update_time) +
		// Preferences.getInstance().get(BundleFlag.RECOM_UPDATE_TIME));
	...
	public void resetListHeader() {
		...
		/*
		 * if (isNeedResetHeader) { if (refreshableView.getAdapter().getCount()
		 * <= refreshableView.getHeaderViewsCount() +
		 * refreshableView.getFooterViewsCount()) { scrollTo(0,
		 * headerScrollToY); } isNeedResetHeader = false; }
		 */
	}
