 ...
 * <p/>
 * <p>Item views may span multiple columns as specified by their {@link com.com.paulz.hhb.view.pulltorefreshgrid.StaggeredGridView.LayoutParams}.
...
public class StaggeredGridView extends ViewGroup {
    ...
    private boolean isOnTouch; // 手指是否接触屏幕
    ...
    protected OnExposeChanged mOnExposeChanged;
    ...
    OnFlingListener mFlingListener;
    ...
    public void setOnExposeChanged(OnExposeChanged changed){
        mOnExposeChanged = changed;
    }
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (mTouchMode == TOUCH_MODE_FLINGING) {
                    ...
                    reportScrollStateChange(OnScrollListener.SCROLL_STATE_TOUCH_SCROLL,getFirstVisiblePosition(),getLastPosition());
                    return true;
                }
                break;
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                //LogUtil.d("----------onTouch ACTION_MOVE-------------");
                float currentY = ev.getRawY();
                if (mMoveTouchListener != null) {
                    if (currentY - mFirstDownY >= 30) {
                        mMoveTouchListener.onMoveDown();
                    } else if (mFirstDownY - currentY >= 30) {
                ...
                if (mFlingListener != null) {
                    mFlingListener.onTouchFling();
                }
                break;

            case MotionEvent.ACTION_CANCEL:
                // LogUtil.d("----------onTouch ACTION_CANCEL-------------");
                isOnTouch = false;

                mTouchMode = TOUCH_MODE_IDLE;
                reportScrollStateChange(OnScrollListener.SCROLL_STATE_IDLE,getFirstVisiblePosition(),getLastPosition());
                updateSelectorState();
                setPressed(false);
            ...
            case MotionEvent.ACTION_UP: {
                ...
                //LogUtil.d("----------onTouch ACTION_UP-------------");
                // LogUtil.d("TAG---------------onTouch ACTION_UP finish-----------" + mScroller.isFinished());
                isOnTouch = false;
                ...
                if (Math.abs(velocity) > mFlingVelocity) { // TODO
                    ...
                    mLastTouchY = 0;
                    reportScrollStateChange(OnScrollListener.SCROLL_STATE_FLING,getFirstVisiblePosition(),getLastPosition());
                    invalidate();
                } else {
                    mIsFling = true;
                    mTouchMode = TOUCH_MODE_IDLE;
        ...
        if (!contentFits) {
            ...
            if (movedBy == 0) {
                if (up) {
                    ...
                    if (mLoadListener != null && getLastPosition() == -1) {  // 滑动到最后一个Item
                        //LogUtil.d("---count---------- = " + getLastPosition());
...
//                    LogUtil.d("---------------------" + getLastPosition());
    ...
    public interface OnExposeChanged{
        public void exposeChanged(int index);
    }
...
//        drawSelector(canvas);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mLastXposition = widthSize;
        mLastYposition = heightSize;
...
//                final int colEnd = Math.min(mColCount, nextCol + lp.span);
    ...
    public interface OnScrollListener{
        ...
        public void onScrollStateChanged(int state, int firstVisibleItem, int lastVisibleItem);
        public void onScroll(int firstVisibleItem, int lastVisibleItem);
    }
    ...
    // 瀑布流取到最后一个item的position，没取到返回-1
    public int getLastPosition () {
        ...
        for (int i = count - 1; i >= 0; i--) {
            ...
            if (child.getVisibility() == View.VISIBLE) {
                ...
                if (frame.contains(mLastXposition-20, mLastYposition -1)||frame.contains(0,mLastYposition-1)) {
    ...
    void reportScrollStateChange(int newState,int first,int last) {
        if (newState != mLastScrollState) {
            if (mOnScrollListener != null) {
                mLastScrollState = newState;
                mOnScrollListener.onScrollStateChanged( newState,first,last);
            }
        }
    }
