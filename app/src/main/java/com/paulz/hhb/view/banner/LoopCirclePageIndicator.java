...
public class LoopCirclePageIndicator extends View implements PageIndicator {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int longPaddingRight = realCount * 15;
        if (mRight) {
            //todo
            longOffset += ((longSize - longPaddingBefore - (longPaddingAfter + longPaddingRight))) - ((realCount * threeRadius) / 2.0f);
        } else {
...
//        }
        ...
        if (isNeedCircle) {
            for (int i = 0; i < realCount; i++) {
                ...
                canvas.drawCircle(dX, dY, pageFillRadius, mPaintPageFill);//每次循环只画一个位置，画n次
            }
        }
    }
