...
//    	append("sb_platform", "android");
    	append("sessionid", HApplication.getInstance().session_id);
        String token= URLEncoder.encode(DESUtil.encrypt(HApplication.getInstance().token+(System.currentTimeMillis()/1000),DESUtil.SECRET_DES_KEY));
        LogUtil.d("-----request----encode token="+token);
    	append("token", token);
	}
    /**
